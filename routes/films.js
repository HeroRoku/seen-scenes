...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const otherFilms = films.filter(
      (film) => !categorizedFilms.includes(film.genreId)
    );
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (authenticated) {
      ...
      const thumbsUp = await Like.findOne({
        where: { userId: req.session.auth.userId, filmId: id },
      });
      const user = await db.User.findByPk(req.session.auth.userId);
      const userReview = await db.Review.findOne({
        where: { userId: user.id, filmId: id },
      });
      res.render("films-id", {
        film,
        reviews,
      ...
      if (!films.length) {
        ...
        try {
          const response = await fetch(
            `https://api.themoviedb.org/3/search/movie?api_key=${movieAPI}&language=en-US&query=${encodedSearchTerm}&page=1&include_adult=false`
          );
...
router.post(
  "/newFilm",
  asyncHandler(async (req, res) => {
    const { id, title, description, releaseDate, posterPath, genreId } =
      req.body;

    const response = await fetch(
      `https://api.themoviedb.org/3/movie/${id}/credits?api_key=${movieAPI}&language=en-US`
    );
